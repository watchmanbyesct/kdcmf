...
const LEVELS = ['introductory','intermediate','advanced','certification']
const defaultCourse = { title:'', slug:'', code:'', description:'', level:'introductory', category:'Episcopal Studies', prerequisites:'', duration_hours:'', price:'0', is_required:false, is_active:true, is_published:false, sort_order:0 }
const defaultLesson = { title:'', content:'', video_url:'', document_url:'', duration_minutes:'', sort_order:0, is_published:false }

function slugify(t: string) { return t.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'') }

export default function AdminAesCourses() {
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('aes_courses').select('*').order('sort_order')
    setCourses(data || [])
    setLoading(false)
  }
  const loadLessons = async (courseId: string) => {
    const { data } = await supabase.from('aes_lessons').select('*').eq('course_id', courseId).order('sort_order')
  ...
  const saveCourse = async () => {
    ...
    if (editingCourse) { await supabase.from('aes_courses').update(payload).eq('id', editingCourse.id) }
    else { await supabase.from('aes_courses').insert(payload) }
  ...
  const saveLesson = async () => {
    ...
    if (editingLesson) { await supabase.from('aes_lessons').update(payload).eq('id', editingLesson.id) }
    else { await supabase.from('aes_lessons').insert(payload) }
    setSaving(false); setShowLessonModal(false); if (activeCourseId) loadLessons(activeCourseId)
  }
  const deleteCourse = async (id: string) => { if (!confirm('Delete this course?')) return; await supabase.from('aes_courses').delete().eq('id', id); load() }
  const deleteLesson = async (id: string, courseId: string) => { if (!confirm('Delete lesson?')) return; await supabase.from('aes_lessons').delete().eq('id', id); loadLessons(courseId) }
  const cf = (k: string, v: any) => setCourseForm((p: any) => ({ ...p, [k]: v }))
  const lf = (k: string, v: any) => setLessonForm((p: any) => ({ ...p, [k]: v }))

  return (
    <AdminLayout title="Academy of Episcopal Studies" subtitle="Courses, lessons, and enrollment management">
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="label">Course Code</label><input className="input" value={courseForm.code} onChange={e => cf('code', e.target.value)} placeholder="AES-101" /></div>
